...
import { Sparkles, Wand2, Copy, Check, Hash, Megaphone, MessageSquareText, Loader2, RotateCcw } from 'lucide-react';
...
export default function CopyStudioPage() {
  ...
  const variationsMut = useMutation({
    ...
    onSuccess: (res) => { setVariations(res.variations); toast.success('5 variations ready'); },
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Copy Studio" description="Generate captions, ad copy and hashtags with Llama 3.1 — streaming live." icon={Sparkles} />
              ...
              <CardBody className="grid gap-3 sm:grid-cols-2">
                {variations.map((v, i) => (
                  <div key={i} className="group relative rounded-xl border border-border p-4 transition hover:border-brand-500/50">
                    ...
                    <Button variant="ghost" size="sm" className="mt-2 opacity-0 group-hover:opacity-100 transition" onClick={() => doCopy(v, i)}>
